...
export async function es_node_import_declaration_single_name_get(es_node_import_specifier) {
  ...
  let specifiers = await m_js_property_get(es_node_import_specifier, "specifiers");
  let specifier = await list_single(specifiers);
  if (specifier.type === "ImportDefaultSpecifier") {
    return;
  }
  let local = await m_js_property_get(specifier, "local");
  let imported = await m_js_property_get(specifier, "imported");

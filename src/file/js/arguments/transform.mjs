...
export async function file_js_arguments_transform(function_name, transformer_arguments) {
  ...
  let transformer = async ast => {
    ...
    await m_js_assert(m_js_equals)(await property_get(statement_first, "type"), "ExpressionStatement");
    let expression_first = await property_get(statement_first, "expression");
    await m_js_assert(m_js_equals)(await property_get(expression_first, "type"), "AwaitExpression");
    let awaited_first = await property_get(expression_first, "argument");
    await m_js_assert(m_js_equals)(await property_get(awaited_first, "type"), "CallExpression");
...
async function es_function_call_to_is(awaited_first_callee, expected_function_name) {
  await m_js_assert(m_js_equals)(await property_get(awaited_first_callee, "type"), "CallExpression");

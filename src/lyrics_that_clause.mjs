    ...
    if (_.some(propositions_for_noun)) {
        if (await random_with_probability(probability_that_clause)) {
            ...
            if (await random_with_probability(probability_object_clause)) {
                let groups_for_object_clause = await list_where(
                    database.groups, 
                    await lyrics_group_is_about(object_clause));
                if (_.some(groups_for_object_clause)) {
                    let group_for_object_clause = await list_item_random(
                        groups_for_object_clause);
                    if (_.some(group_for_object_clause.objects)) {
                        let group_for_object_clause_noun = await list_item_random(
                            group_for_object_clause.objects);
                        object_clause = `a ${
                            group_for_object_clause_noun
                        } ${await list_item_random(['like', 'as'])} ${object_clause}`;
                    }
                }
            }

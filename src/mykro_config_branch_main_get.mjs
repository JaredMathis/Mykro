import {mykrodev_config_save} from "./mykro_config_save.mjs";
...
export async function mykrodev_config_branch_main_get() {
  ...
  await mykrodev_config_property_exists_ensure(config, config, "branch", {});
  await mykrodev_config_property_exists_ensure(config, config.branch, "main", "main");
  return config.branch.main;
}
async function mykrodev_config_property_exists_ensure(config, object, property_name, default_value) {
